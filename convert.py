...
def rename(name):
    ...
    if name.startswith('model.encoder.resnet'):
        ...
        name = name.replace('layer3', 'layer4').replace('layer2', 'layer3').replace('layer1', 'layer2').replace('layer0', 'layer1')
        name = name.replace('conv_relu_dropblock_0.conv2d', 'conv1').replace('conv_relu_dropblock_1.bn', 'bn1')
        name = name.replace('conv_relu_dropblock_3.conv2d', 'conv2').replace('conv_relu_dropblock_4.bn', 'bn2')
        name = name.replace('conv_relu_dropblock_6.conv2d', 'conv3').replace('conv_relu_dropblock_7.bn', 'bn3')
    ...
    for g, v1 in proj_dict.items():
        for t, v2 in v1.items():
            ...
            if t == 'weight':
                ...
                raise ValueError(f'Invalid parameter specified, excepted to be weight or bias, but got {t}')
...
def adjust(state_dict):
    ...
    state_dict = {k: np.transpose(v) if 'weight' in k else v for k, v in state_dict.items()}
...
def convert(model, tf_ckpt_path):
    ...
    for name, shape in init_vars:
        if 'optimizer' not in name and 'global_step' not in name and '_CHECK' not in name and 'save_counter' not in name:
